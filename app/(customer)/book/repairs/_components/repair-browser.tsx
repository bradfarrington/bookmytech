...
import { MAX_REPAIRS_PER_BOOKING } from "@/lib/bookings/repair-ids";
import { buildRepairHrefs, parseCrumbs } from "@/lib/bookings/repair-hrefs";
...
import { RepairSearch } from "./repair-search";
...
// straight into the existing match → slot → pay funnel. A search box above
// the list (Task 30) finds a job by name without drilling.
//
...
// different prices for the same job on the same car. The rows themselves live
// in repair-rows.tsx and the links in lib/bookings/repair-hrefs.ts, shared
// with the search box for the same reason.
...
export async function RepairBrowser({
  ...
  const trail = parseCrumbs(crumbs);
          ...
          {trail.map((c, i) => (
            <span key={`${c.id}-${i}`} className="flex items-center gap-1.5">
              ...
              {i === trail.length - 1 ? (
                ...
                <Link href={hrefs.crumbHref(i)} className="text-brand-blue hover:underline">
                  {c.label}
                </Link>
              )}
            </span>
