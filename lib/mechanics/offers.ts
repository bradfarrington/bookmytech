...
// Accepting and declining a job offer, shared by the website's server actions
// (app/actions/job-offers.ts) and the mechanic app's route handlers
// (app/api/mobile/v1/mechanic/offers/[id]/*).
//
...
/**
 * Why a refusal was refused. The website shows `error` and ignores this; the
 * mobile routes turn it into a status code, because the app tells "someone else
 * got it" (409) apart from every other failure.
 */
export type OfferRefusalCode = "not_found" | "forbidden" | "conflict" | "failed";
...
export type OfferResult =
  | {
      ...
      bookingId?: string;
      /**
       * The accepted booking is an ALL-DAY one (which includes every flexible
       * booking), so the mechanic should be taken straight to picking a 2-hour
       * arrival window (Task 21).
       */
      needsArrivalWindow?: boolean;
    }
  | { ok: false; code: OfferRefusalCode; error: string };

// Both mutations check the offer belongs to `mechanicId`, then perform
...
export async function acceptOfferFor(mechanicId: string, offerId: string): Promise<OfferResult> {
  ...
  if (!offer) return { ok: false, code: "not_found", error: "That offer no longer exists." };
  if (offer.mechanic_id !== mechanicId)
    return { ok: false, code: "forbidden", error: "This isn't your offer." };
  if (offer.response)
    return { ok: false, code: "conflict", error: "You've already responded to this offer." };
  ...
  const { data: claimed, error: claimErr } = await admin
    .from("bookings")
    .update({ mechanic_id: mechanicId, status: "confirmed" })
  ...
  if (claimErr) return { ok: false, code: "failed", error: claimErr.message };
  ...
  if (!claimed) {
    ...
    return { ok: false, code: "conflict", error: "Another mechanic just accepted this job." };
  }
...
export async function declineOfferFor(mechanicId: string, offerId: string): Promise<OfferResult> {
  const admin = createAdminClient();
