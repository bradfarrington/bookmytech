...
**Status:** ✅ Complete (2026-06-05) — Stage 1 (rebooking + reminders, `0023`) · Stage 3 (referrals + account credits, `0024`). **Stage 2 (Mechanic Pro tier) was moved to [Task 13 — Final integrations](13-final-integrations.md)** (owner decision 2026-06-05): the Pro tier depends on the dispatch layer + mature ratings/disputes data and is cleaner to switch on last, alongside the SMS infrastructure. The stage headings below keep their original **Stage 1 / Stage 3** numbers so cross-references elsewhere still resolve; there is intentionally no Stage 2 here. **Owner decision (2026-06-05): the pre-auth hold is always taken at booking — the Trusted-Customer skip-pre-auth was dropped** (see Stage 3). ⚠️ Apply migrations `0023`–`0024`; the credit/`free` path wasn't live-fired (see Stage 3 notes).
...
- [x] Credits applied automatically at booking checkout — `prepareCheckout` computes `applied = min(balance, total)` server-side; the pre-auth hold / PI is reduced by it; `createBookingAction` redeems it via a ledger row and snapshots `credit_applied_pence`. Mechanic payout + platform fee stay computed on the full total (credit is platform-funded).
- [~] Trusted Customer status logic — **dropped by owner decision (2026-06-05).** Its only concrete benefit was skipping the pre-auth (see next), which was rejected, so the Trusted Customer tier carries no behaviour and was removed (no badge, no `isTrusted`).
- [~] Skip pre-auth flow for trusted customers (capture only on completion) — **dropped by owner decision (2026-06-05): the pre-authorisation hold is ALWAYS taken at booking, trusted or not.** The deferred/SetupIntent "save card, charge later" path was removed. The only no-card case left is `free` — when account credit covers the **whole** total there is genuinely nothing to authorise.

**Implementation notes / deviations:**
- **The pre-auth hold is always taken at booking.** `bookings.payment_mode` is just `preauth` (default — manual-capture hold of `total − credit`, captured on completion) or `free` (credit ≥ total → nothing to authorise, no card). There is **no** trusted/deferred skip-the-hold mode.
- Credit reduces the **amount held**, never the mechanic payout / platform fee (credit is platform-funded).
...
- ⚠️ Migration `0024` adds `profiles.referral_code/referred_by` (+ backfill), `customer_credits`, and `bookings.credit_applied_pence` + `payment_mode`. **Apply `0024`.** ⚠️ The credit redemption + `free` path weren't live-fired against Stripe this session — build + 39 unit tests pass; the guest pre-auth path is byte-for-byte unchanged. Exercise a credited booking before relying on it.
