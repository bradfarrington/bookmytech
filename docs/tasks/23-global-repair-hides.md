...
- **Model page** (`/admin/vehicles/[make]/[model]?tab=repairs`): an **"Apply changes to: This model only / All vehicles"** picker beside the breadcrumbs (`scope-picker.tsx`; `?scope=global`, kept across drill-down and engine-variant changes). In model scope every toggle stays live and shows the *effective* state for that model, with a caption where the switch alone can't explain it: 🔒 "Hidden for all vehicles", "Shown for this model · hidden elsewhere", "Hidden from customers". In global scope the toggle shows the global hide itself, under an amber banner, with a caption when this model overrides it. `RepairToggle` takes `target` instead of make/model names.
- **Review page** `/admin/vehicles/hidden`: every global hide (description, node id, since date, and "Shown on: …" listing the models that override it) with a **Show again** button (`show-again-button.tsx` — refreshes rather than the optimistic tick, so the row leaves the list), then the per-model hides grouped by model. `/admin/vehicles` gets a one-line "{n} repairs hidden for all vehicles · Review" card. Top-bar crumbs for `/admin/vehicles` (missing before) and `/admin/vehicles/hidden`.
...
2. `/admin/vehicles` → Volkswagen → Golf VII → Repair times. Set "Apply changes to" to **All vehicles** → amber banner. Switch off **Body - Exterior** and, under Brakes (Mechanical) → Front brakes, **Renew the front brake pads**. Both ticks empty.
3. Back to **This model only** → both dimmed with 🔒 "Hidden for all vehicles". Switch front pads **on** → caption "Shown for this model · hidden elsewhere".
4. `/admin/vehicles/hidden` → both listed; front pads shows "Shown on: VOLKSWAGEN Golf VII"; the index card says "2 repairs hidden for all vehicles".
