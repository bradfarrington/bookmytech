...
> **Outage visibility added 2026-08-26.** The demo licence expired on 2026-08-09 and nobody noticed for sixteen days, because the integration degrades silently *by design* — the funnel told every customer "we couldn't match your registration" (blaming their reg for our lapsed licence) and the only signal was a `console.error`. Since Task 17 the repair catalogue IS the booking flow, so "HaynesPro is refusing our credentials" means "nobody can book anything".
>
> - `lib/haynespro/health.ts` — the auth path now records what it saw (`ok` / `auth_failed` / `unreachable`, plus HaynesPro's statusCode and a plain-English reading of it) into `platform_settings.haynespro_health`. Shared across serverless instances, same reasoning as the VRID row; a module-level flag would be per-instance and useless.
> - `/admin/vehicles` shows it: a red "booking is down" banner on `auth_failed`, amber on unreachable/unconfigured, a quiet "connected" line otherwise.
> - The customer message changed. **The wire `code` did NOT** — it stays `vehicle_not_matched` on purpose, because that field is what shipped app builds branch on and they have never heard of any other value. An additive optional `retryable?: true` marks the outage case for clients that want it. Verified live: the endpoint returns the new copy with the code unchanged.
> - `health.ts` deliberately has **no `import "server-only"`** — `client.ts` imports it at module top and is itself imported by the unit tests for its pure helpers, so the guard would break `npm test` while buying nothing (everything there takes its Supabase client as a parameter).
...
- **Task 23 (2026-09-04)** added an "All vehicles" scope to those toggles (wildcard rows, `mode` column via `0054`) with per-model overrides, and verified live that a node id is the same job on every make — see `docs/tasks/23-global-repair-hides.md`.
