...
| scheduled_at               | timestamptz | nullable until slot picked. The arrival window's **start** (ordering, day grouping, the 24-hour cancellation-fee boundary). On a flexible booking (Task 28) it is 8am on the **earliest** offered day until the mechanic picks |
| slot_window                | text        | 0028 — the arrival window the customer picked, verbatim: `"8am–10am"` … `"6pm–8pm"` or `"All day (8am–8pm)"` (`lib/slots.ts`). Null on legacy rows and once anyone sets an exact time (customer reschedule, accepted proposal). A mechanic narrowing an all-day booking (Task 21) rewrites it to the 2-hour label in place |
| candidate_days             | date[]      | 0057 — the several all-day dates a flexible customer offered (Task 28), as UK calendar dates; CHECK null or ≥ 2. Set only with the all-day `slot_window`. Nulled when the mechanic picks a day + window (`setArrivalWindow`) or anyone sets an exact time. `isFlexibleBooking` / `formatBookingWhen` in `lib/slots.ts` are the readers |
...
### `promo_codes` / `promo_redemptions` / `promo_code_sends` — `0063` (Task 35)

Discount codes the admin creates and sends out. `promo_codes`: `code` (unique, uppercase, `^[A-Z0-9-]{3,24}$`), `kind` (`percent` | `fixed`), `value`, window (`starts_at` / `expires_at`), `max_redemptions` (null = unlimited), `per_customer_limit`, `min_total_pence`, `is_active`. `promo_redemptions`: one row per use — `reserved` while the customer is at the card step (keyed on the PaymentIntent, one-hour TTL), `redeemed` once the booking exists, `released` when the hold was abandoned; reserved-but-unexpired rows count toward the caps. `promo_code_sends`: the send log. `bookings.discount_pence` + `promo_code` record what was applied.

**`redeem_promo_code(code_id, customer_id, discount_pence, payment_intent_id, booking_id)`** (SECURITY DEFINER, revoked from anon/authenticated) is the **single enforcement point** for both caps: it locks the code row with `for update` before counting, so two checkouts can't take the last redemption. Application code never counts to decide.

Applied at checkout **before** account credit (`chargeAfterDiscounts` in `lib/promos/validate.ts`): `charge = total − discount − credit`. **BMT-funded** (owner decision, Brad 2026-09-08) — `mechanic_payout_pence` is untouched, so the discount comes out of the platform fee. `completeAndCharge` captures the discounted figure and `refundBooking` measures refundable the same way. RLS: admin-only on all three; the customer never reads them (`lib/promos/apply.ts` resolves a typed code through the service-role client and answers with a sentence).

### `booking_faults` / `job_quotes` / `job_quote_lines` — `0062` (Task 33)

Faults the mechanic notes on a job, and quotes for extra work. `job_quotes.kind`: `now` (extra work on this visit — approved with a second manual-capture Stripe hold, captured with the base hold at completion) or `follow_on` (a return visit — approval leads into the funnel, Task 34). Every quote raises the price and needs the customer's approval; a mechanic-side price reduction was built and then removed on 2026-09-08 — what Gareth actually wanted is a *revision* of the job, `job_revisions` below (Task 37). A revision's positive difference is carried by a `job_quotes` row (kind `now`, title "Revised job — difference") born `draft` and flipped straight to `approved` with the customer's card; that is the only way a `draft` row is used. `status`: `sent` → `approved` | `declined` | `withdrawn` | `expired` (7 days, cron). The platform figures are snapshotted on the quote (`hourly_rate_pence`, the booking's `commission_rate`, `labour_pence`, `parts_pence`, `total_pence`, `platform_fee_pence`, `mechanic_payout_pence`); `stripe_payment_intent_id` / `stripe_charge_id` / `captured_at` track its hold. **On approval the booking's `total_pence`, `base_price_pence`, `parts_price_pence`, `platform_fee_pence` and `mechanic_payout_pence` are incremented in place** — the app reads `total_pence` as "what you pay". Lines: `labour` (hours × rate, `node_id` when HaynesPro's book time filled the hours), `part` (`part_id` from the catalogue, or typed), `other`; `fault_id` links back to the fault quoted for. `bookings.source_quote_id` — set on a booking made from a `follow_on` quote (Task 34), whose `follow_on_booking_id` points back at it; such a booking is priced from the quote's snapshot, takes its vehicle and `preferred_mechanic_id` from the quote's original job, writes the quote's labour lines to `booking_repairs` (`node_id` may be `q:<line id>` when no HaynesPro job backed it) and its part lines to **`booking_parts`**. RLS: SELECT for the booking's customer, the mechanic, admins; no write policies (`lib/quotes/*` via service role). `job_quotes` is on Realtime. Events: `fault_added`, `quote_sent`, `quote_approved` (payload before/after), `quote_declined`, `quote_withdrawn`, `quote_expired`; `payment_captured` with `quote_id`; `payout_transferred` with `source_charge`; `payment_refunded` with `payment_intent_id`.

### `job_revisions` — `0064` (Task 37)

The mechanic's on-site rewrite of the job sheet: they arrive, find the booked repair isn't what the car needs, and propose a different job at its new price, up or down. `status`: `sent` → `approved` | `declined` | `withdrawn` | `expired` (24 h, the quotes cron). `reason` (why the booked repair isn't right — the customer reads it), `note`. **`before` / `after`** are jsonb snapshots of the job sheet in one shape (`lib/revisions/snapshot.ts`): `repairIds` (the catalogue ids as chosen), `lines` (nodeId, description, raw/charged hours, linePence, kind job|product, itemId/itemLabel for a combined repair), `parts` (`id` = the `booking_parts` row when it already existed, partId, name, quantity, unitPence, linePence, sourcing), `repairDescription`, `serviceDurationHours`, `rawHours`, `combineSource`, `oil`, `hourlyRatePence`, `commissionRate`, and the five money figures that map 1:1 onto the bookings columns (`basePricePence`, `partsPricePence`, `totalPence`, `platformFeePence`, `mechanicPayoutPence`). `after` is priced by `quoteRepairs` at the **booking's snapshotted** rate and commission (an additive override, Task 37); `before` is the booking's own figures with any approved Task 33 extra work subtracted. `after_repair_ids`, `before_total_pence`, `after_total_pence`, `difference_pence` (CHECK `= after − before`), **`hold_quote_id`** → `job_quotes` (CHECK: set exactly when the difference is positive on a `sent` row — the draft `now` quote that carries the difference), `sent_at` / `responded_at` / `expires_at`. **On approval the booking IS the revised job**: `booking_repairs` replaced (one line → no rows), `booking_parts` reconciled (kept rows keep sourcing/status), `repair_node_id`, `repair_description`, `combine_source`, `service_duration_hours`, `vehicle_raw_duration_hours`, `engine_oil_*` and the five money columns set to `after` + approved extras — so `completeAndCharge` captures the base hold for `total − Σ approved quotes = before`, unchanged. **One approved revision per booking.** Every revision needs the customer's approval, cheaper or dearer (owner decision 2026-09-09). If declined, `endJobOnSite` captures the on-site diagnostic fee (`platform_settings.on_site_diagnostic_fee_pence`, seeded 5999), the en-route cancellation fee, or nothing, from the base hold; booking → `cancelled` (no new status) with the `cancelled` event carrying `outcome: 'customer_declined_revision'`, `revision_id`, `fee_kind`, `fee_pence`; `payment_captured` `kind: 'on_site_diagnostic' | 'on_site_cancellation'`; the fee is paid out to the mechanic minus commission through `lib/payments/payout.ts`. RLS: SELECT for the booking's customer, the mechanic, admins; no write policies (`lib/revisions/*` via service role). On Realtime. Events: `revision_sent`, `revision_approved` (payload before/after/added/removed), `revision_declined`, `revision_withdrawn`, `revision_expired`.
...
**Mechanic Inbox, case evidence, dispute evidence and private notes (`0085`, Task 69).** `mechanic_inbox_reads` (`mechanic_id` PK, `read_before`, `read_ids text[]` ≤ 200) + `mark_mechanic_inbox_item_read` / `mark_mechanic_inbox_all_read` — a mirror of `customer_inbox_reads` (0075). `resolution_cases.photos text[]` (≤ 6). `dispute_messages.photos text[]` (≤ 6) and `dispute_messages.visible_to` (`'mechanic' | 'customer'`, null = everyone, admin rows only): the **"Parties read dispute thread" policy hides a private note from the other party**. `0085` also creates the three `resolution_*` tables as `0032` defines them, in case `0032` never reached production — **do not run `0032` itself**, its `booking_events` CHECK is out of date. The `resolution_reasons` SELECT policy is now "has a `mechanics` row, or is admin".
