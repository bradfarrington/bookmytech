...
**CORRECTED 2026-09-16.** This paragraph originally ended "the only outstanding Stripe work is a test-card run … a verification step rather than a change." **That was wrong, and it was wrong twice, because it stayed here after being apologised for elsewhere.** It described the BACKEND, in a document addressed to the app, where it read as a statement about the whole system — and at the time the app's entire saved-cards UI was a stub returning `[]`. The corrected version:

**Nothing to configure at Stripe, for the owner.** Saved cards use SetupIntents, Customers, PaymentMethods and CustomerSessions, all generally available on the pinned API version `2026-04-22.dahlia`. **No webhook is involved** — the Stripe webhook handles only `account.updated` for mechanic Connect onboarding. Stripe attaches a card to the Customer when the SetupIntent succeeds, and cards are listed from Stripe directly, so there is nothing to subscribe to and nothing to reconcile. **That is a statement about the backend and about Brad's Stripe dashboard only. It says nothing about whether the app's saved-cards screens are built**, and when it was written they were not.
