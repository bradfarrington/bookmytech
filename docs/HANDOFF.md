...
**Next up: Task 05 — Mechanic dashboard** (`docs/tasks/05-mechanic-dashboard.md`). The mechanic-facing app: job offers, accept/decline, schedule, en-route/start/complete lifecycle, earnings. This is the counterpart to the admin side just built — mechanics currently only get the "dashboard coming soon" landing at `/mechanic`. Read the task doc before starting.
