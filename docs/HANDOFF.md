...
**In progress: Task 05 — Mechanic dashboard** (`docs/tasks/05-mechanic-dashboard.md`). Stages 1–4 ✅ — auth + light shell, jobs page (KPIs + live broadcast offers + dispatch), daily schedule + SVG service-area map, and the **job-detail view** at `/mechanic/jobs/[id]` (cancel + reschedule-propose, mechanic-side; customer accept/decline of a reschedule + the "replacement accepted" email are stubbed for Task 09). **Next: Stage 5 — earnings page**, then Stage 6 (availability + profile). ⚠️ Apply `0009_mechanic_job_actions.sql` before testing Stage 4 (adds `customer_phone`/`cancellation_reason`/`reschedule_*` to `bookings`, extends `booking_events` enum).
